...
import { agentCards, features, permissionModes, providerList } from '../lib/content'
import type { Feature } from '../lib/content'
import { card, cardHover, chip, container, heading, line, muted } from '../lib/styles'
...
function SpotlightCard({
  ...
  return (
    <div
      ref={ref}
      onMouseMove={onMove}
      className={cn('group relative overflow-hidden', card, cardHover, className)}
    >
      <div className="pointer-events-none absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100 bg-[radial-gradient(260px_circle_at_var(--spot-x,50%)_var(--spot-y,50%),rgba(132,154,114,0.16),transparent_70%)]" />
...
const ACCENT_TILE: Record<NonNullable<Feature['accent']>, string> = {
  brand: 'bg-brand-500/12 text-brand-600 dark:bg-brand-400/12 dark:text-brand-300',
  accent: 'bg-accent-500/12 text-accent-600 dark:bg-accent-400/12 dark:text-accent-300',
  neutral: 'bg-zinc-900/[0.05] text-zinc-600 dark:bg-white/[0.06] dark:text-zinc-300',
}
  ...
  return (
    <div className="mt-6 flex flex-wrap gap-2">
      {agentCards.map((agent) => (
        ...
        >
          <agent.icon className="h-3 w-3 text-brand-500" />
          {agent.name}
        </span>
      ))}
      <span className="inline-flex items-center gap-1.5 rounded-full border border-zinc-200/80 bg-white px-2.5 py-1 text-[11px] font-medium text-zinc-700 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-zinc-300">
        <Bot className="h-3 w-3 text-accent-500" />
        teammates
      </span>
    </div>
  )
...
export function Features() {
  return (
    <Section id="features">
      ...
      <div className={cn(container, 'mt-14 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3')}>
        {features.map((feature, index) => (
          ...
          >
            <SpotlightCard className="h-full p-6 sm:p-7">
              ...
              <h3 className={cn('mt-5 text-lg font-semibold tracking-[-0.02em]', heading)}>
                {feature.title}
              </h3>
              <p className={cn('mt-2.5 text-sm leading-relaxed', muted)}>{feature.description}</p>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <p className={cn('text-sm', muted)}>
              Built with TypeScript, React + Ink and a WASM glob engine — MIT licensed.
            </p>
          </div>
          <a
            href="#tools"
