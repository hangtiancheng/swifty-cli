...
import { HardDrive, GitBranch, ShieldAlert, ShieldCheck, Lock, Wrench, ListTree, Zap } from 'lucide-react'
...
interface DialogView {
  ...
  title: string
  meta: string
  rows: { label: string; value: string; tone: 'allow' | 'ask' | 'deny' | 'auto' }[]
  note: string
}
...
export function Safety() {
  ...
  return (
    <Section id="safety">
      ...
      <div className={cn(container, 'mt-14 grid grid-cols-1 gap-6 lg:grid-cols-[1fr_1.05fr]')}>
        <Reveal>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    selected
                      ...
                      : cn(line, 'bg-white hover:border-zinc-300 dark:bg-white/[0.02] dark:hover:border-white/20'),
                  )}
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className={cn('mt-1 text-sm leading-relaxed', muted)}>{mode.description}</p>
                      <p className="mt-1 text-xs text-zinc-400 dark:text-zinc-600">{mode.detail}</p>
                    </div>
                  </div>
                </button>
              )
        ...
        <Reveal delay={0.1}>
          <div className={cn('sticky top-24 overflow-hidden p-1', card)}>
            ...
            <p className={cn('px-4 py-3 text-center text-xs', muted)}>{view.note}</p>
          </div>
        </Reveal>
      </div>

      <div className={cn(container, 'mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3')}>
        {SAFETY_FEATURES.map((feature, index) => (
          <Reveal key={feature.title} delay={index * 0.06}>
            <div className={cn('h-full rounded-2xl border p-5 dark:bg-white/[0.02]', line)}>
              <feature.icon className="h-5 w-5 text-brand-500 dark:text-brand-400" />
              <h3 className={cn('mt-4 text-sm font-semibold', heading)}>{feature.title}</h3>
              <p className={cn('mt-2 text-sm leading-relaxed', muted)}>{feature.body}</p>
            </div>
          </Reveal>
        ))}
      </div>
      ...
      <Reveal delay={0.1} className={cn(container, 'mt-4')}>
        ...
        >
          <span className="inline-flex items-center gap-2 text-zinc-500 dark:text-zinc-400">
            ...
            <code className="font-mono text-xs text-zinc-700 dark:text-zinc-300">.swifty/</code>
          </span>
          <span className="inline-flex items-center gap-2 text-zinc-500 dark:text-zinc-400">
