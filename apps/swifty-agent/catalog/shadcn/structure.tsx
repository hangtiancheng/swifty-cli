      ...
      items: z
        .array(
          z.object({
            ...
            child: ComponentIdSchema.describe("The ID of the section content component."),
          }),
        )
        .min(1)
...
export const Accordion = createComponentImplementation(AccordionApi, ({ props, buildChild }) => {
  const items = (Array.isArray(props.items) ? props.items : []) as AccordionItemDef[];
...
export const ButtonGroupApi = {
  ...
  schema: z
    .object({
      ...WEIGHT,
      children: ChildListSchema.describe("The IDs of the grouped Button components."),
...
export const CollapsibleApi = {
  ...
  schema: z
    .object({
      ...
      child: ComponentIdSchema.describe("The ID of the collapsible content component."),
...
export const ResizableApi = {
  ...
  schema: z
    .object({
      ...WEIGHT,
      direction: z.enum(["horizontal", "vertical"]).default("horizontal").optional(),
      panels: z
        .array(
          z.object({
            child: ComponentIdSchema.describe("The ID of the panel content component."),
            defaultSize: z.number().describe("The initial size as a percentage.").optional(),
          }),
        )
        .min(2)
...
export const Resizable = createComponentImplementation(ResizableApi, ({ props, buildChild }) => {
  const panels = (Array.isArray(props.panels) ? props.panels : []) as PanelDef[];
    ...
    >
      {panels.map((panel, i) => (
        <Fragment key={i}>
          ...
          <ResizablePanel
            defaultSize={
              typeof panel.defaultSize === "number" ? String(panel.defaultSize) : undefined
            }
          >
            <div className="p-3">
              {typeof panel.child === "string" ? buildChild(panel.child) : null}
            </div>
          </ResizablePanel>
        </Fragment>
      ))}
...
export const TableApi = {
  ...
  schema: z
    .object({
      ...WEIGHT,
      caption: DynamicStringSchema.describe("Optional table caption.").optional(),
      columns: z
        .array(
          z.object({
      ...
      <UITable>
        ...
        <TableBody>
          {rows.map((row, ri) => (
            <TableRow key={ri}>
              {columns.map((col, ci) => (
                <TableCell key={ci}>{String(row?.[String(col.key)] ?? "")}</TableCell>
              ))}
            </TableRow>
          ))}
        </TableBody>
