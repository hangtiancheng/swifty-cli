...
export const Calendar = createComponentImplementation(CalendarApi, ({ props }) => {
...
export const Combobox = createComponentImplementation(ComboboxApi, ({ props }) => {
  ...
  return (
    <UIField style={weightStyle(props.weight)}>
      ...
      <UICombobox
        ...
        value={props.value ? (labelByValue.get(String(props.value)) ?? null) : null}
        onValueChange={(label) => {
          if (typeof label === "string") props.setValue(valueByLabel.get(label) ?? label);
        }}
      >
...
export const CommandApi = {
  ...
  schema: z
    .object({
      ...WEIGHT,
      placeholder: z.string().describe("The search input placeholder.").optional(),
      groups: z
        .array(
          z.object({
...
export const Command = createComponentImplementation(CommandApi, ({ props }) => {
  const groups = (Array.isArray(props.groups) ? props.groups : []) as CommandGroupDef[];

  return (
    <UICommand className="w-full rounded-lg border" style={weightStyle(props.weight)}>
      <CommandInput placeholder={props.placeholder ?? "Type a command or search..."} />
      <CommandList>
        <CommandEmpty>No results found.</CommandEmpty>
        {groups.map((group, i) => (
          <CommandGroup key={i} heading={group.heading ? String(group.heading) : undefined}>
            {((group.items ?? []) as CommandItemDef[]).map((item, j) => (
              <CommandItem
                key={j}
                onSelect={
                  typeof item.action === "function" ? (item.action as () => void) : undefined
                }
              >
                {String(item.label ?? "")}
              </CommandItem>
...
export const FieldApi = {
  ...
  schema: z
    .object({
      ...
      description: DynamicStringSchema.describe("The field description.").optional(),
      child: ComponentIdSchema.describe("The ID of the field control component."),
...
export const InputGroupApi = {
  ...
  schema: z
    .object({
      ...
      placeholder: z.string().describe("The input placeholder.").optional(),
      prefixText: z.string().describe("Static text shown before the input.").optional(),
      suffixText: z.string().describe("Static text shown after the input.").optional(),
      buttonLabel: DynamicStringSchema.describe("Label of the trailing button.").optional(),
      action: ActionSchema.describe("Action dispatched by the trailing button.").optional(),
    })
    .strict(),
};

export const InputGroup = createComponentImplementation(InputGroupApi, ({ props }) => (
  <UIInputGroup style={weightStyle(props.weight)}>
    ...
    {(props.suffixText || props.buttonLabel) && (
      <InputGroupAddon align="inline-end">
        {props.suffixText && <InputGroupText>{props.suffixText}</InputGroupText>}
        {props.buttonLabel && (
          <InputGroupButton onClick={props.action}>{props.buttonLabel}</InputGroupButton>
        )}
      </InputGroupAddon>
    )}
  </UIInputGroup>
...
export const InputOtpApi = {
  ...
  schema: z
    .object({
      ...WEIGHT,
      length: z.number().min(1).max(12).describe("The number of digits.").default(6).optional(),
      value: DynamicStringSchema.describe(
...
export const InputOtp = createComponentImplementation(InputOtpApi, ({ props }) => {
...
export const NativeSelect = createComponentImplementation(NativeSelectApi, ({ props }) => (
  <UIField style={weightStyle(props.weight)}>
    {props.label && <FieldLabel>{props.label}</FieldLabel>}
    <UINativeSelect value={props.value || ""} onChange={(e) => props.setValue(e.target.value)}>
