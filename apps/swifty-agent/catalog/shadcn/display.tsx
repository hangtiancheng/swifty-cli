...
export const AvatarApi = {
  ...
  schema: z
    .object({
      ...WEIGHT,
      src: DynamicStringSchema.describe("The image URL of the avatar.").optional(),
...
export const Badge = createComponentImplementation(BadgeApi, ({ props }) => (
  <UIBadge variant={props.variant ?? "default"} style={weightStyle(props.weight)}>
    <CatalogIcon name={props.icon} />
    {props.text}
  </UIBadge>
));
...
export const EmptyApi = {
  ...
  schema: z
    .object({
      ...
      description: DynamicStringSchema.describe("The empty state description.").optional(),
      icon: ICON_NAME.optional(),
...
export const Empty = createComponentImplementation(EmptyApi, ({ props, buildChild }) => (
...
export const ItemApi = {
  ...
  schema: z
    .object({
      ...
      description: DynamicStringSchema.describe("The item description.").optional(),
      icon: ICON_NAME.optional(),
      child: ComponentIdSchema.describe(
        "Optional ID of a child component rendered as trailing actions.",
      ).optional(),
      variant: z.enum(["default", "outline", "muted"]).default("default").optional(),
    })
    .strict(),
};

export const Item = createComponentImplementation(ItemApi, ({ props, buildChild }) => (
  <UIItem variant={props.variant ?? "default"} style={weightStyle(props.weight)}>
    {props.icon && (
...
export const KbdApi = {
  ...
  schema: z
    .object({
      ...WEIGHT,
      keys: z.array(z.string()).min(1).describe("The keyboard keys to display, e.g. ['Cmd', 'K']."),
    })
    .strict(),
};
...
export const ProgressApi = {
  ...
  schema: z
    .object({
      ...WEIGHT,
      value: DynamicNumberSchema.describe("The progress value between 0 and 100."),
      label: DynamicStringSchema.describe("Optional label shown above the bar.").optional(),
      showValue: z.boolean().describe("Whether to display the percentage value.").optional(),
    })
    .strict(),
};
...
export const ScrollAreaApi = {
  ...
  schema: z
    .object({
      ...WEIGHT,
      child: ComponentIdSchema.describe("The ID of the scrollable child component."),
      maxHeight: z.number().describe("The maximum height in pixels before scrolling.").optional(),
    })
    .strict(),
};

export const ScrollArea = createComponentImplementation(ScrollAreaApi, ({ props, buildChild }) => (
  <UIScrollArea
    className="w-full"
...
export const SkeletonApi = {
  ...
  schema: z
    .object({
      ...WEIGHT,
      width: z.number().describe("Width in pixels; omit for full width.").optional(),
      height: z.number().describe("Height in pixels.").optional(),
      shape: z.enum(["rectangle", "circle", "text"]).default("rectangle").optional(),
    })
    .strict(),
};
...
export const Skeleton = createComponentImplementation(SkeletonApi, ({ props }) => (
  <UISkeleton
    className={cn(props.shape === "circle" && "rounded-full", props.shape === "text" && "h-4")}
    style={{
...
export const Spinner = createComponentImplementation(SpinnerApi, ({ props }) => (
  <UISpinner
    style={{
      ...weightStyle(props.weight),
      width: props.size,
