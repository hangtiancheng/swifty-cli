...
export const Breadcrumb = createComponentImplementation(BreadcrumbApi, ({ props }) => {
  const items = (Array.isArray(props.items) ? props.items : []) as MenuEntryDef[];
      ...
      menus: z
        .array(
          z.object({
            ...
            items: z.array(MENU_ENTRY).min(1).describe("The entries of this menu."),
          }),
        )
        .min(1)
...
export const Menubar = createComponentImplementation(MenubarApi, ({ props }) => {
...
export const NavigationMenu = createComponentImplementation(
  NavigationMenuApi,
  ({ props, buildChild }) => {
    const items = (Array.isArray(props.items) ? props.items : []) as NavItemDef[];
          ...
          {items.map((item, i) => (
            <NavigationMenuItem key={i}>
              {typeof item.child === "string" ? (
                <>
                  <NavigationMenuTrigger>{String(item.label ?? "")}</NavigationMenuTrigger>
                  <NavigationMenuContent>{buildChild(item.child)}</NavigationMenuContent>
                </>
              ) : (
                <NavigationMenuLink
                  className="cursor-pointer"
...
export const Pagination = createComponentImplementation(PaginationApi, ({ props }) => {
  const total = typeof props.totalPages === "number" ? props.totalPages : 1;
  const page = Math.min(Math.max(typeof props.page === "number" ? props.page : 1, 1), total);
  const go = (next: number) => props.setPage(Math.min(Math.max(next, 1), total));

  return (
    <UIPagination style={weightStyle(props.weight)}>
      <PaginationContent>
        <PaginationItem>
          <PaginationPrevious className="cursor-pointer" onClick={() => go(page - 1)} />
        </PaginationItem>
        ...
        <PaginationItem>
          <PaginationNext className="cursor-pointer" onClick={() => go(page + 1)} />
        </PaginationItem>
      </PaginationContent>
    </UIPagination>
  );
