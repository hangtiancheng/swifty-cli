...
function MarkdownText({ text, style }: { text: string; style: CSSProperties }) {
  ...
  return (
    <div className={MARKDOWN_CLASSES} style={style} dangerouslySetInnerHTML={{ __html: html }} />
  );
}
...
export const Text = createComponentImplementation(TextApi, ({ props }) => {
  ...
  const variantClass = props.variant ? VARIANT_CLASSES[props.variant] : undefined;
