...
export const DateTimeInput = createComponentImplementation(DateTimeInputApi, ({ props }) => {
  ...
  const disabledDays = [
    ...(minParts.date ? [{ before: new Date(`${minParts.date}T00:00:00`) }] : []),
    ...(maxParts.date ? [{ after: new Date(`${maxParts.date}T00:00:00`) }] : []),
  ];

  // A time bound only applies on the bound's own day (or in time-only mode).
  const timeBound = (bound: { date: string; time: string }) =>
    bound.time && (!enableDate || (date && date === bound.date)) ? bound.time : undefined;

  const commit = (nextDate: string, nextTime: string) => {
    if (enableDate && enableTime) {
      props.setValue(nextDate || nextTime ? `${nextDate}T${nextTime || "00:00"}` : "");
      return;
    }
  ...
  return (
    <Field>
      {props.label && <FieldLabel htmlFor={enableTime ? id : undefined}>{props.label}</FieldLabel>}
