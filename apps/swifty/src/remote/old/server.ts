...
export class RemoteServer {
  ...
  constructor(
    ...
    hookCfgs: HookConfig[],
    addr: string,
    enableCoordinatorMode: boolean = false,
    forkDisabled: boolean = false,
  ) {
  ...
  private async initAgent(): Promise<void> {
    ...
    // fork 模式宿主：声明了 mode: fork 的 Skill 跑在隔离的子 Agent 里，
    // SOP 正文只出现在子 Agent 的对话中，主对话拿到的是最终结果
    const skillForkHost: SkillForkHost = {
      activateSkill: skillHost.activateSkill,
      snapshotParentMessages: (count: number) => {
        const msgs = this.conv?.getMessages() ?? [];
        return msgs
          .slice(-count)
    ...
    // 注册 LoadSkill 工具
    this.registry.register(new LoadSkillTool(catalog, skillHost, skillForkHost));
    ...
    // 注册 Team 相关工具。teamRunAgentFactory 接收队友专属注册表（含团队任务板工具），
    // 返回运行该队友 agent 主循环的回调。
    ...
    this.registry.register(new TaskStopTool(this.teamManager));
    this.registry.register(new SyntheticOutputTool());
    ...
    const agentTool = new AgentTool(
      ...
      async (prompt, _conversation, registry, modelOverride?) => {
        ...
        const { buildSystemPrompt: buildSP, detectEnvironment: detectEnv } =
    ...
    // 把团队管理器接入 AgentTool，让 team_name 队友路径生效（队友会拿到团队共享任务板工具）
    agentTool.forkDisabled = this.forkDisabled;
    agentTool.setTeamManager(this.teamManager, teamRunAgentFactory);
  ...
  private async runAgentLoop(): Promise<void> {
    ...
    const agent = new Agent({
      ...
      toolFilter: (name: string) => {
        // Skill 过滤和 coordinator 收窄要同时成立，任一挡下就不放行
      ...
      onLoopComplete: (conv) => {
        ...
        const summary = conv
          ...
          .join("\n");
        new MemoryExtractor(this.client, workDir).extract(summary).catch(() => {});
        // 后台记忆整理：合并重复、删除过时、修正矛盾
        new MemoryConsolidator(this.client, workDir, {
          appendSystem: (msg) => conv.addSystemReminder(msg),
        })
          .maybeRun()
          .catch(() => {});
      },
    ...
    for await (const event of agent.run()) {
      switch (event.type) {
        ...
        case "compact":
          ...
          // 持久化压缩边界
          if (event.boundary) {
            sessionMod.saveCompactBoundary(workDir, this.sessionId, event.boundary);
          }
          break;
