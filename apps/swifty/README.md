...
Pasting an image saves it as a PNG under `.swifty/file-history/` and inserts the file path into the prompt; the agent reads it on demand with the ReadFile tool. Linux requires `wl-clipboard` (Wayland) or `xclip` (X11).
